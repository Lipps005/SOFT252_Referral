 ...
 * @author samue
 */
public class LoadUsers implements ILoadAllBehaviour {
   ...
   public DefaultListModel LoadAll(ListableDirectory directory) {
      ...
      try
        {    
            ...
            {
               File userSubFolder = new File(directory.GetDirectory()+file.getName());
               File[] userListOfFiles = userSubFolder.listFiles();
               for(File userfile : userListOfFiles)
               {
               if (userfile.isFile()) 
               {
